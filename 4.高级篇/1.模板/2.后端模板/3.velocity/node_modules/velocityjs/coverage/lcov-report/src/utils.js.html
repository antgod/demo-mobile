...
utils.mixin = function(to, from) {
  utils.forEach(from, function(val, key) {
    if (utils.isArray(val) || utils.isObject(val)) {
      to[key] = utils.mixin(val, to[key] || <span class="branch-1 cbranch-no" title="branch not covered" >{})</span>;
    } else {
      to[key] = val;
    }
  });
...
utils.indexOf = function(elem, arr) {
  <span class="missing-if-branch" title="else path not taken" >E</span>if (utils.isArray(arr)) {
